...
main() {
	...
	must.link "$HOME/.dotfiles/user/scripts" "$HOME/scripts"
	...
	local -a files=(~/.dotfiles/.usr/bin/*)
	core.shopt_pop
	...
	local file=; for file in "${files[@]}"; do
		ln -fs  "$file" ~/.dotfiles/.usr/bin
	done; unset -v file

	ln -s "$HOME/.dotfiles/.home/Documents/Programming/repos/choose/target/debug/choose" ~/.dotfiles/.usr/bin/choose
	must.link ~/.dotfiles/.dotmgr/bin/dotmgr ~/.dotfiles/.usr/bin/dotmgr
	if [ "$profile" = 'desktop' ]; then
		must.link ~/repos/dotfox/dotfox ~/.dotfiles/.usr/bin/dotfox
	fi
	must.link ~/Documents/repos/basalt/pkg/bin/basalt ~/.dotfiles/.usr/bin/basalt
	...
	if [ "$profile" = 'desktop' ]; then
		cp -f "$HOME/.dotfiles/user/.config/user-dirs.dirs/user-dirs-custom.conf" "$XDG_CONFIG_HOME/user-dirs.dirs"
		...
		cp -f "$HOME/.dotfiles/user/.config/user-dirs.dirs/user-dirs-default.conf" "$XDG_CONFIG_HOME/user-dirs.dirs"
