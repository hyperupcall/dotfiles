...
if [[ $FONT_SIZE == "" ]]; then
	...
	exit    1
elif [[ $FONT_SIZE =~ ^[0-9]+([.][0-9]+)?$  ]] || [[ $FONT_SIZE =~ ^-?[0-9]+([.][0-9]+)?$  ]]; then
...
#y-pos default=0
if [[ $Y_POS =~ ^[0-9]+([.][0-9]+)?$ ]] || [[ $Y_POS =~ ^-?[0-9]+([.][0-9]+)?$ ]]; then #y-pos in pixels
	v=${Y_POS%.*}
	Y_POS=$v
elif [[ $Y_POS =~ ^[0-9]+([.][0-9]+)?%$ ]]; then #y-pos in percentage
	v=$(  echo $Y_POS | sed 's/^-//g; s/%$//g')
	let y_position=(${FONT_SIZE} * ${v%.*})/100
	Y_POS=$y_position
elif [[ $Y_POS =~ ^-?[0-9]+([.][0-9]+)?%$ ]]; then #y-pos in percentage(negative)
	v=$(  echo $Y_POS | sed 's/^-//g; s/%$//g')
	let y_position=(${FONT_SIZE} * ${v%.*})/100
