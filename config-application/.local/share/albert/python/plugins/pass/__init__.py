...
class Plugin(PluginInstance, TriggerQueryHandler):
		...
		return [
			{
				'type': 'checkbox',
				'property': 'use_otp',
				'label': 'Enable pass OTP extension',
			},
			{
				'type': 'lineedit',
				'property': 'otp_glob',
				...
				StandardItem(
					...
					actions=[
						Action(
							'copy',
							'Copy',
							lambda pwd=password: runDetachedProcess(
								['pass', 'otp', '--clip', pwd]
							),
						),
					],
	...
	def getPasswords(self, otp=False):
		...
		for root, dirnames, filenames in os.walk(PASS_DIR, followlinks=True):
			for filename in fnmatch.filter(
				filenames, self._otp_glob if otp else '*.gpg'
			):
				passwords.append(
	...
	def getPasswordsFromSearch(self, otp_query, otp=False):
		passwords = [
			password
			for password in self.getPasswords(otp)
			if otp_query.strip().lower() in password.lower()
		]
		return passwords
