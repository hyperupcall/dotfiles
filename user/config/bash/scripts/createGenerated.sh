...
profileDir="$XDG_CONFIG_HOME/profile"

# ------------------------- Main ------------------------- #
# user Functions
exec 6> "$generatedDir/.bashrc-user-functions.sh"
util_print_autogen_info >&6
find "$XDG_CONFIG_HOME/profile/fns/" -ignore_readdir_race -type f -name "*.sh" \
...
util_print_autogen_info >&6
find "$XDG_CONFIG_HOME/profile/aliases/" -ignore_readdir_race -type f -name "*.sh" \
		-exec sh -c 'cat < $0' {} \; \
